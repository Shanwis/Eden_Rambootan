...
const Dashboard = () => {
  ...
  const fetchIndividualData = async () => {
    console.log('🔄 Fetching individual data sources...');
    ...
    try {
      ...
      if (redditResponse.ok) {
        ...
        if (redditData.sentiment_analysis && redditData.sentiment_analysis.total > 0) {
          ...
          reputationScore += (positive / total) * 100 * 0.3; // 30% weight
          totalMentions += total;
          sourceCount++;
        }
      }
    ...
    try {
      const newsResponse = await fetch(`http://localhost:5001/api/news?company=${encodeURIComponent(companyData.company)}&type=${encodeURIComponent(companyData.type)}`);
      if (newsResponse.ok) {
        const newsData = await newsResponse.json();
        dataSources.news = newsData;
        setDataStatus(prev => ({ ...prev, news: newsData.is_real_data || false }));
        
        if (newsData.sentiment_analysis && newsData.sentiment_analysis.total > 0) {
          const { positive, total } = newsData.sentiment_analysis;
          reputationScore += (positive / total) * 100 * 0.25; // 25% weight
          totalMentions += total;
          sourceCount++;
        }
      }
    ...
    try {
      ...
      if (mastodonResponse.ok) {
        ...
        dataSources.mastodon = mastodonData;
        setDataStatus(prev => ({ ...prev, mastodon: mastodonData.is_real_data || false }));
        
        if (mastodonData.posts && mastodonData.posts.length > 0) {
          const positiveCount = mastodonData.posts.filter(post => post.favourite_count > post.reply_count).length;
          reputationScore += (positiveCount / mastodonData.posts.length) * 100 * 0.2; // 20% weight
          totalMentions += mastodonData.posts.length;
          sourceCount++;
        }
      }
    } catch (error) {
      console.log('❌ Mastodon API not available');
    }

    try {
      const reviewsResponse = await fetch(`http://localhost:5003/api/reviews`, {
        method: 'POST',
      ...
      if (reviewsResponse.ok) {
        ...
        dataSources.reviews = reviewsData;
        setDataStatus(prev => ({ ...prev, reviews: reviewsData.is_real_data || false }));
        
        if (reviewsData.sentiment_analysis && reviewsData.sentiment_analysis.total > 0) {
          const { positive, total } = reviewsData.sentiment_analysis;
          reputationScore += (positive / total) * 100 * 0.25; // 25% weight
          totalMentions += total;
          sourceCount++;
        }
    ...
    const finalReputationScore = sourceCount > 0 ? Math.round(reputationScore / sourceCount) : 75;
    
    // Generate dynamic source breakdown
    const sourceBreakdown = [];
    if (dataSources.reddit?.sentiment_analysis?.total) {
      sourceBreakdown.push({
        name: 'Reddit',
        value: dataSources.reddit.sentiment_analysis.total,
        color: '#FF6B6B'
      });
    }
    if (dataSources.news?.sentiment_analysis?.total) {
      sourceBreakdown.push({
        name: 'News',
        value: dataSources.news.sentiment_analysis.total,
        color: '#45B7D1'
      });
    }
    if (dataSources.mastodon?.posts?.length) {
      sourceBreakdown.push({
        name: 'Mastodon',
        value: dataSources.mastodon.posts.length,
        color: '#4ECDC4'
      });
    }
    if (dataSources.reviews?.sentiment_analysis?.total) {
      sourceBreakdown.push({
        name: 'Reviews',
        value: dataSources.reviews.sentiment_analysis.total,
        color: '#96CEB4'
      });
    }

    // Calculate percentages
    const totalBreakdown = sourceBreakdown.reduce((sum, source) => sum + source.value, 0);
    sourceBreakdown.forEach(source => {
      source.percentage = totalBreakdown > 0 ? Math.round((source.value / totalBreakdown) * 100) : 0;
    });

    // Generate dynamic trend data based on actual sentiment
    const trendData = [];
    const basePositive = Math.max(30, Math.min(70, finalReputationScore));
    const baseNeutral = Math.max(20, Math.min(40, 100 - basePositive - 20));
    const baseNegative = Math.max(10, Math.min(30, 100 - basePositive - baseNeutral));
    ...
    for (let i = 1; i <= 7; i++) {
      ...
      trendData.push({
        ...
        neutral: Math.max(0, Math.min(100, baseNeutral + Math.floor(Math.random() * 6) - 3)),
        negative: Math.max(0, Math.min(100, baseNegative + Math.floor(Math.random() * 6) - 3))
      });
    }
    ...
    const unifiedData = {
      ...
      unified_insights: `Analysis for ${companyData.company} based on ${sourceCount} data sources. Reputation score: ${finalReputationScore}/100. Focus on improving customer engagement and addressing service quality concerns.`,
